...
def test_evaluate_sampled_negatives_perfect_recommender():
    ...
    out = evaluate_sampled_negatives(_Perfect(),
                                     histories={"u0": ["b9"]},
                                     relevance={"u0": {"b0"}},
                                     all_items=["b0", "b1", "b2", "b3", "b4"],
                                     n_neg=3,
                                     k=10,
                                     seed=0)
...
def test_evaluate_sampled_negatives_accepts_popularity_weights():
    ...
    out = evaluate_sampled_negatives(_Perfect(),
                                     histories={"u0": ["b9"]},
                                     relevance={"u0": {"b0"}},
                                     all_items=["b0", "n1", "n2", "n3"],
                                     n_neg=2,
                                     k=10,
                                     seed=0,
                                     item_weights=[1, 5, 5,
                                                   5])  # popularity-weighted negatives path
    assert out["recall@10"] == 1.0
    assert set(out) == {"recall@10", "ndcg@10", "mrr"}
...
def test_evaluate_sampled_negatives_random_recommender_is_low():
    ...
    out = evaluate_sampled_negatives(_Zero(),
                                     histories={"u0": ["b9"]},
                                     relevance={"u0": {"b0"}},
                                     all_items=["b0"] + [f"n{i}" for i in range(50)],
                                     n_neg=20,
                                     k=1,
                                     seed=0)
...
def test_cooccurrence_picks_most_coread():
    df = _inter([("u0", "a"), ("u0", "x"), ("u0", "y"), ("u1", "a"), ("u1", "x"), ("u1", "z"),
                 ("u2", "a"), ("u2", "x")])
...
def test_popularity_diagnostics_popular_recs_score_high_percentile():
    ...
    out = popularity_diagnostics(_FixedRec(["p0", "p1"]), {"u": []}, ranking, catalog_size=4, k=10)
